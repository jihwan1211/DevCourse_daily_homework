...
const bestBooksData: Book[] = Array.from({ length: 10 }).map((item, index) => ({
  ...
  category_id: faker.helpers.rangeToNumber({ min: 100, max: 200 }),
  form: "종이책",
  isbn: faker.commerce.isbn(),
  summary: faker.lorem.paragraph(),
  detail: faker.lorem.paragraph(),
  author: faker.person.firstName(),
  pages: faker.helpers.rangeToNumber({ min: 100, max: 200 }),
  contents: faker.lorem.paragraph(),
  price: faker.helpers.rangeToNumber({ min: 10000, max: 50000 }),
  likes: faker.helpers.rangeToNumber({ min: 0, max: 200 }),
  pubDate: faker.date.past().toISOString(),
}));

export const bestBooks = http.get("/books/best", () => {
